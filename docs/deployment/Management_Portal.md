...
- **Same XDP mode** — mgmt-br must use `xdpgeneric` (same as physical ports) for `BPF_F_BROADCAST` redirect to work across all ports.

## Requirements

- Kernel 5.15+ (for `BPF_F_BROADCAST` with `BPF_MAP_TYPE_DEVMAP`)
- `dhcpcd` installed (for DHCP mode)
- Physical ports attached in `xdpgeneric` mode
...
4. Attaches XDP dispatcher to `mgmt-br` in SKB/generic mode
5. **mgmtd** (`rswitch-mgmtd`) starts in the namespace
6. Detects loader-managed `mgmt-br`, skips veth creation
7. Runs `dhcpcd` on `mgmt0` — DHCP traffic flows through XDP pipeline
8. Begins serving HTTP on `0.0.0.0:<port>`
...
Both should show `xdpgeneric`. If mgmt-br shows `xdp` (native mode), the XDP broadcast redirect silently fails.
